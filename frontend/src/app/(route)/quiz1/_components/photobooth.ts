...
export abstract class PhotoBooth {
	...
	static get_media(): Promise<MediaStream> {
		...
		const constraints = { audio: false, video: { facingMode: PhotoBooth.mode } }  
	...
	static take_photo() {
		...
		const context = PhotoBooth.canvas.getContext('2d')
		context!.drawImage(PhotoBooth.video, 0, 0, PhotoBooth.canvas.width, PhotoBooth.canvas.height)
		
		const url = PhotoBooth.canvas.toDataURL('image/jpeg')
