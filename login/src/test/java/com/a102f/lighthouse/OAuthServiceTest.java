...
import com.a102f.lighthouse.auth.service.OAuthClient;
import com.a102f.lighthouse.auth.service.OAuthService;
...
public class OAuthServiceTest {
    ...
    public void kakaoOAuthLoginTest() {
        ...
        when(memberInfoResponse.getProfileUrl()).thenReturn("test_profile_image");

        // 가짜 회원 객체에 대한 반환 값 설정
        when(member.getMemberId()).thenReturn("123");
        when(member.getNickname()).thenReturn("test_nickname");
        when(member.getProfileImage()).thenReturn("test_profile_image");
        ...
        assertEquals(loginResponseDto.getProfileImage(), member.getProfileImage());
    }
}
